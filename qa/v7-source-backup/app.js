(() => {
  ...
  function renderArticle() {
    ...
    $('#article-source-count').textContent = `${(draft.sources || topic.sources).length} 条来源`;
  ...
  function renderWorks() {
    ...
    $('#work-list').innerHTML = state.works.map((work) => {
      ...
      return `<article class="work-card ${confirmed ? 'confirmed-work' : 'note-work'}">
        ...
        <div class="work-sources">保留 ${(work.sources || topic.sources).length} 条收藏来源</div>
  ...
  function openPublishConfirmation(work) {
    ...
    showDialog('把这颗果实放入同题森林？', `<p>文章已经确认结果，目前仍是私人保存。再次确认后，它会出现在“${esc(topicById(work.topicId).short)}”的演示树林中。</p><div class="dialog-note">将展示：文章标题、确认后的正文和 ${(work.sources || topicById(work.topicId).sources).length} 条来源根系。不会真的发布到知乎。</div><p>发布和文章确认是两个独立动作，你之后可以取消发布。</p><div class="dialog-actions"><button class="quiet-button" data-action="close-dialog">继续私人保存</button><button class="primary-button" data-action="confirm-publish" data-id="${esc(work.id)}">确认放入森林 →</button></div>`);
  }
  ...
  function route() {
    ...
    if (!['home', 'garden', 'topic', 'favorites', 'thinking', 'article', 'forest', 'works'].includes(view)) view = 'home';
    if (view === 'article' && !state.draftArticle) view = state.session ? 'thinking' : 'works';
    $$('.view').forEach((section) => { section.hidden = section.id !== `view-${view}`; });
    document.body.dataset.view = view;
    $$('[data-route]').forEach((link) => {
      const active = link.dataset.route === view || (view === 'garden' && link.dataset.route === 'thinking') || (view === 'article' && link.dataset.route === 'works') || (view === 'forest' && link.dataset.route === 'works');
    ...
    syncNavCounts();
    const titles = { home: '知树 · 从真实收藏里发现值得写的问题', garden: '我的知树 · 生长空间', topic: `${currentTopic.short} · 知树`, favorites: '我的根系 · 知树', thinking: '一起想清楚 · 知树', article: '确认文章，让它结果 · 知树', forest: '同题森林 · 知树', works: '我的作品 · 知树' };
  ...
  window.ZhishuGardenBridge = {
    getSources() {
      const personal = state.userSources.map(source => ({ ...source, personal: true }));
      const demo = DATA.topics[0].sources.map(source => ({ id: source.id, title: source.title, content: source.summary, url: source.url, personal: false }));
      return { personal, demo };
    },
    addSources(sources) {
      for (const source of sources) if (!state.userSources.some(item => item.id === source.id)) state.userSources.unshift({ ...source, createdAt: new Date().toISOString() });
      save(); syncNavCounts();
    },
    getWork(id) { return state.works.find(work => work.id === id); },
    saveArticle(payload) {
      const topic = topicById(payload.topicId);
      const work = { id: payload.id || `garden-work-${Date.now()}`, topicId: topic.id, title: payload.title, body: payload.body, viewpoint: payload.viewpoint, answers: [...payload.answers], presetUsed: [...payload.presetUsed], sourceIds: [...payload.sourceIds], sources: payload.sources.map(source => ({...source})), status: 'confirmed', confirmedAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
      const index = state.works.findIndex(item => item.id === work.id);
      if (index >= 0) state.works[index] = work; else state.works.unshift(work);
      state.publishedWorkIds = state.publishedWorkIds.filter(id => id !== work.id);
      state.session = { topicId: topic.id, round: 3, reached: 3, answers: [...payload.answers], presetUsed: [...payload.presetUsed], updatedAt: work.updatedAt };
      state.draftArticle = { ...work };
      state.forestTopicId = topic.id;
      save(); return work.id;
    },
    publish(workId, topicId) {
      const work = state.works.find(item => item.id === workId);
      if (!work || work.status !== 'confirmed') return false;
      work.topicId = topicById(topicId || work.topicId).id;
      if (!state.publishedWorkIds.includes(workId)) state.publishedWorkIds.push(workId);
      state.forestTopicId = work.topicId;
      if (state.draftArticle?.id === work.id) state.draftArticle = {...work};
      save(); return true;
    },
    openForest() { location.hash = 'forest'; }
  };
