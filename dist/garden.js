(() => {
  ...
  function render(animate=false){
    ...
    if(animate&&!matchMedia('(prefers-reduced-motion: reduce)').matches){world.classList.add('growing');clearTimeout(growTimer);const sprout=world.classList.contains('sprout');const video=world.querySelector('.sprout-video');if(sprout&&video){let settleTimer=0;const settle=()=>{world.classList.add('sprout-settle');};const arm=()=>{clearTimeout(settleTimer);const rate=video.playbackRate||1;const left=Math.max(0,3.4-video.currentTime)*1000/rate;settleTimer=setTimeout(settle,left);};video.addEventListener('playing',arm);video.addEventListener('ratechange',arm);video.addEventListener('timeupdate',()=>{if(video.currentTime>=3.4)settle();});const finish=()=>{clearTimeout(settleTimer);settle();world.classList.add('sprout-done');clearTimeout(growTimer);growTimer=setTimeout(()=>world.classList.remove('growing'),900);};video.addEventListener('ended',finish,{once:true});video.addEventListener('error',finish,{once:true});let began=false;const reveal=()=>{video.playbackRate=1;world.classList.add('sprout-live');arm();};const go=()=>{if(began)return;began=true;const p=video.play?.();if(p&&p.then)p.then(()=>{if(video.currentTime>=0.85){reveal();return;}video.playbackRate=4;const iv=setInterval(()=>{if(video.currentTime>=0.88||video.ended||video.paused){clearInterval(iv);reveal();}},25);}).catch(finish);else reveal();};const canSeek=()=>{try{return video.seekable.length>0&&video.seekable.end(video.seekable.length-1)>=0.95;}catch(e){return false;}};const start=()=>{if(began)return;if(!canSeek())return;video.addEventListener('seeked',go,{once:true});try{video.currentTime=0.9;}catch(e){go();}};['loadedmetadata','loadeddata','canplay','canplaythrough','progress','durationchange'].forEach(e=>video.addEventListener(e,start));start();setTimeout(go,350);growTimer=setTimeout(finish,6500);}else if(world.dataset.stage==='roots'&&world.querySelector('.root-system')){const rs=world.querySelector('.root-system');rs.addEventListener('animationstart',()=>{clearTimeout(growTimer);growTimer=setTimeout(()=>world.classList.remove('growing'),2150);},{once:true});growTimer=setTimeout(()=>world.classList.remove('growing'),4500);}else growTimer=setTimeout(()=>world.classList.remove('growing'),1050);}
    persist();
  }
