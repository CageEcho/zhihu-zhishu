...
function taperedRoot([x0,y0,x1,y1,x2,y2,x3,y3],w0,w1){const L=[],R=[],N=18;for(let i=0;i<=N;i++){const t=i/N,u=1-t;const x=u*u*u*x0+3*u*u*t*x1+3*u*t*t*x2+t*t*t*x3,y=u*u*u*y0+3*u*u*t*y1+3*u*t*t*y2+t*t*t*y3;const dx=3*u*u*(x1-x0)+6*u*t*(x2-x1)+3*t*t*(x3-x2),dy=3*u*u*(y1-y0)+6*u*t*(y2-y1)+3*t*t*(y3-y2);const len=Math.hypot(dx,dy)||1,nx=-dy/len,ny=dx/len;const w=(w0+(w1-w0)*Math.pow(t,.85))/2;L.push(`${(x+nx*w).toFixed(1)} ${(y+ny*w).toFixed(1)}`);R.unshift(`${(x-nx*w).toFixed(1)} ${(y-ny*w).toFixed(1)}`);}return `M${L.join('L')}L${R.join('L')}Z`;}
const rootStrands=[[[735,652,733,690,724,738,716,808],11,2.2],[[733,668,708,690,634,728,574,747],8.5,3],[[574,747,552,752,528,768,506,790],3,1],[[737,666,770,690,824,740,852,757],8.5,3],[[852,757,874,762,896,778,918,802],3,1],[[727,712,714,738,698,752,694,763],5.5,2.4],[[694,763,688,776,676,792,662,812],2.4,.8],[[724,752,748,772,776,790,806,814],4.5,1],[[738,700,758,716,772,744,776,780],4,1],[[640,724,626,742,614,762,606,788],3,.8],[[800,720,824,738,842,770,846,798],3.2,.8]];
function rootSystem(){let g='<g class="root-system">';rootStrands.forEach(([b,w0,w1])=>{const d=taperedRoot(b,w0,w1);g+=`<path d="${d}" fill="#3a2716" opacity=".28" transform="translate(0 2.5)"/>`;});rootStrands.forEach(([b,w0,w1])=>{const d=taperedRoot(b,w0,w1);g+=`<path d="${d}" fill="url(#rootColor)" stroke="#9d7f56" stroke-width=".7" stroke-opacity=".6" stroke-linejoin="round"/>`;});rootStrands.forEach(([b,w0,w1])=>{if(w0<5)return;const d=taperedRoot(b,Math.max(w0*.32,1.2),Math.max(w1*.3,.5));g+=`<path d="${d}" fill="#f6ead0" opacity=".38" transform="translate(-.6 -1)"/>`;});return g+'</g>';}
