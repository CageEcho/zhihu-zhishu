...
world.innerHTML=`<header class="topbar"><a class="brand" href="#home" aria-label="知树首页">知树</a><span class="divider"></span><span class="garden-tree-title" title="${esc(window.ZhishuTreeStore.title(garden))}">${esc(garden.sources.length?window.ZhishuTreeStore.title(garden):'一颗新的种子')}</span><nav class="garden-main-nav" aria-label="主导航"><a href="#garden" aria-current="page">我的知树</a><a href="#history">历史记录</a><a href="#forest">同题树林</a></nav><span class="private">${lock}本次演示保存</span><span class="avatar">我</span></header><div class="garden-stage"><div class="landscape"><div class="horizon"></div><div class="cloud a"></div><div class="cloud b"></div><i class="speck s1"></i><i class="speck s2"></i><i class="speck s3"></i><i class="speck s4"></i></div><div class="intro ${center?'central':''}"><div class="eyebrow"><i></i>${['SEED / 播种','ROOT / 扎根','BRANCH / 抽枝','LEAF / 展叶','FRUIT / 结果'][rank]}</div><h1>${rank>=2?data[0].replace('，','，<br>'):data[0]}</h1><p>${data[1]}</p>${rank>0?`<div class="stage-note">${stage==='roots'?`已导入 ${garden.sources.length} 条收藏`:stage==='branches'?`${garden.answers.filter(x=>x.trim()).length} / 3 轮对话已完成`:data[2]}</div>`:`<p style="font-size:12px;margin-top:5px;color:#a6b2b6">${stage==='roots'?`已导入 ${garden.sources.length} 条收藏`:stage==='branches'?`${garden.answers.filter(x=>x.trim()).length} / 3 轮对话已完成`:data[2]}</p>`}</div><img class="tree-art" src="assets/${stage==='seed'?'soil-seed-v2.png':stage==='roots'?'soil-roots-v3-wide.png':rank>=3?'tree-full.png':'tree-bare.png'}" alt="${stage==='seed'?'土地与发光种子':stage==='roots'?'横向铺满的绿色地景与立体自然根系':rank>=3?'枝叶与自然根系完整呈现的知树':'参考图中的树干与自然根系'}">${sceneSVG()}${renderTags()}<div class="hint-bar">${['轻触种子，开启生长','每条根系都是收藏中的一个观点','枝干连接你的理由、回应与边界','每片树叶，都保留着你的一轮思考','文章完成，果实才会成熟'][rank]}</div></div><footer class="footer"><span class="footer-left"><i></i>从收藏开始，让思考自然生长。</span><div class="growth-dots">${[0,1,2,3,4].map(i=>`<i class="${i===rank?'on':i<rank?'done':''}"></i>`).join('')}</div><span id="garden-save-status">${rank===0?'等待种下一颗想法':'已保存至本次演示'}</span></footer>`;
