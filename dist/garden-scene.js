...
<radialGradient id="fruitRed" cx=".38" cy=".36" r=".78"><stop stop-color="#ff7466"/><stop offset=".38" stop-color="#ee2f34"/><stop offset=".74" stop-color="#c4151f"/><stop offset="1" stop-color="#7a0b13"/></radialGradient>
...
<filter id="appleSoft" x="-1" y="-1" width="3" height="3"><feGaussianBlur stdDeviation="2.6"/></filter>
</defs>`;
...
function fruit(x,y,scale=1){return `<g transform="translate(${x} ${y}) scale(${scale})"><ellipse cy="6" rx="52" ry="48" fill="url(#fruitHalo)"/><path d="M0-31C-7-41-33-40-38-15C-43 11-27 39 0 39C27 39 43 11 38-15C33-40 7-41 0-31Z" fill="url(#fruitRed)" filter="url(#smallshadow)"/><path d="M0-31C-7-41-33-40-38-15C-43 11-27 39 0 39C27 39 43 11 38-15C33-40 7-41 0-31Z" fill="url(#fruitShade)"/><path d="M-30 14C-24 30-10 39 4 38" fill="none" stroke="#ff9d8e" stroke-width="3" opacity=".32" stroke-linecap="round"/><ellipse cx="-14" cy="-11" rx="10" ry="15" fill="#fff" opacity=".55" transform="rotate(-18 -14 -11)" filter="url(#appleSoft)"/><ellipse cx="-13" cy="-13" rx="4.5" ry="8" fill="#fff" opacity=".85" transform="rotate(-18 -13 -13)"/><circle cx="-7" cy="-24" r="2.4" fill="#fff" opacity=".95"/><ellipse cx="0" cy="-29" rx="8" ry="3.5" fill="#8a0d15" opacity=".5"/><path d="M0-30Q1-40 4-48" fill="none" stroke="#5b3a1f" stroke-width="4.5" stroke-linecap="round"/><path d="M-1-32Q0-40 2-46" fill="none" stroke="#a3764a" stroke-width="1.4" stroke-linecap="round" opacity=".7"/></g>`;}
...
function rootSystem(){const k=720/1254*1.14,cx=735,top=660;const w=830*k,h=230*k,x=cx-w/2+3,y=top;return `<g class="root-system"><image href="assets/roots-overlay.png" x="${x.toFixed(1)}" y="${y}" width="${w.toFixed(1)}" height="${h.toFixed(1)}" preserveAspectRatio="xMidYMid meet"/><image class="root-glow" href="assets/roots-overlay.png" x="${x.toFixed(1)}" y="${y}" width="${w.toFixed(1)}" height="${h.toFixed(1)}" preserveAspectRatio="xMidYMid meet"/></g>`;}
...
function renderTags(){
 ...
 if(stage==='leaves')h+=`<button type="button" class="article-guide-orb" data-garden-action="article-guide" aria-label="三轮思考已完成，生成文章"><span class="article-guide-bloom"></span><span class="article-guide-ring r1"></span><span class="article-guide-ring r2"></span><span class="article-guide-core"><svg class="article-guide-apple" viewBox="0 0 72 72" aria-hidden="true"><path d="M36 27c-7-7-19-7-24 3C5 44 14 63 27 67c4 1 7-1 9-3 2 2 5 4 9 3 13-4 22-23 15-37-5-10-17-10-24-3Z" fill="#d9a325"/><path d="M36 26c-1-8 2-13 7-17" fill="none" stroke="#b8801a" stroke-width="4" stroke-linecap="round"/><path d="M40 15c6-8 14-7 18-3-5 6-12 7-18 3Z" fill="#d9a325"/></svg></span></button>`;
 displayFruits.forEach((item,i)=>{
 const [x,y]=fruitPoints[i];
...
const center=rank<2;
const gardenImage=stage==='seed'?'soil-seed-v4-transparent.png':stage==='roots'?'soil-seedless.png':rank>=3?'tree-full-tree.png':'tree-bare-trunk.png';
...
world.innerHTML=`<header class="topbar"><a class="brand" href="#home" aria-label="知树首页">知树</a><span class="divider"></span><span class="garden-tree-title" title="${esc(window.ZhishuTreeStore.title(garden))}">${esc(garden.sources.length?window.ZhishuTreeStore.title(garden):'一颗新的种子')}</span><nav class="garden-main-nav" aria-label="主导航"><a href="#garden" aria-current="page">我的知树</a><a href="#history">历史树轮</a><a href="#forest">同题树林</a></nav><span class="private">${lock}本次演示保存</span><span class="avatar">我</span></header><div class="garden-stage"><div class="landscape"><div class="horizon"></div><div class="cloud a"></div><div class="cloud b"></div><i class="speck s1"></i><i class="speck s2"></i><i class="speck s3"></i><i class="speck s4"></i></div><div class="intro ${center?'central':''}"><div class="eyebrow"><i></i>${['SEED / 播种','ROOT / 扎根','BRANCH / 抽枝','LEAF / 展叶','FRUIT / 结果'][rank]}</div><h1>${rank>=2?data[0].replace('，','，<br>'):data[0]}</h1><p>${data[1]}</p>${rank>0?`<div class="stage-note">${stage==='roots'?`已导入 ${garden.sources.length} 条收藏`:stage==='branches'?`${garden.answers.filter(x=>x.trim()).length} / 3 轮对话已完成`:data[2]}</div>`:`<p style="font-size:12px;margin-top:5px;color:#a6b2b6">${stage==='roots'?`已导入 ${garden.sources.length} 条收藏`:stage==='branches'?`${garden.answers.filter(x=>x.trim()).length} / 3 轮对话已完成`:data[2]}</p>`}</div>${canopyFrom==='clusters'&&stage==='leaves'?`<div class="canopy-ghost" aria-hidden="true"><img class="tree-art" src="assets/tree-bare-trunk.png" alt=""><svg class="scene-svg" viewBox="0 0 1440 900">${canopyLayers(garden.answers)}</svg></div>`:''}${enterStage==='branches'&&stage==='branches'?`<div class="sprout-clip" aria-hidden="true"><video class="sprout-video" src="assets/sprout-to-trunk.mp4" muted playsinline preload="auto"></video></div>`:''}${rank>=2?`<img class="soil-art" src="assets/soil-seedless.png" alt="">`:''}${stage==='roots'?`<img class="seed-art" src="assets/soil-seed-v4-transparent.png" alt="" aria-hidden="true">`:''}<img class="tree-art" src="assets/${gardenImage}" alt="${gardenImageAlt}">${rank>0?`<svg class="roots-svg" viewBox="0 0 1440 900" aria-hidden="true">${rootSystem()}</svg>`:''}${sceneSVG()}${renderTags()}<div class="hint-bar">${['轻触种子，开启生长','每条根系都是收藏中的一个观点','枝干连接你的理由、回应与边界','每片树叶，都保留着你的一轮思考','文章完成，果实才会成熟'][rank]}</div></div><footer class="footer"><span class="footer-left"><i></i>从收藏开始，让思考自然生长。</span><div class="growth-dots">${[0,1,2,3,4].map(i=>`<i class="${i===rank?'on':i<rank?'done':''}"></i>`).join('')}</div><span id="garden-save-status">${rank===0?'等待种下一颗想法':'已保存至本次演示'}</span></footer>`;
